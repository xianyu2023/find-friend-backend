...
public class UserController {
    ...
    public BaseResponse<Boolean> updateUser(@RequestBody UserUpdateRequest userUpdateRequest,
        ...
        //补充校验更新的内容
        userService.updateUserJudge(user);
    ...
    public BaseResponse<List<UserVO>> listUserByTag(@RequestParam(required = false) List<String> tagNameList) {
        if (CollectionUtils.isEmpty(tagNameList)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        List<UserVO> userList = userService.listUserByTagSqlOrMemory(tagNameList);
    ...
    public BaseResponse<Page<UserVO>> recommendUsers(PageRequest pageRequest, HttpServletRequest request) {
        ...
        User loginUser = userService.getLoginUser(request);
        //读缓存
        Page<UserVO> userVOPage = userService.readRedisCache(loginUser.getId());
        //有缓存
        if (userVOPage != null) {
            return ResultUtils.success(userVOPage);
        }
        //没有缓存时，先查询数据库把查询结果返回，并把查询数据写入到缓存
        userVOPage = userService.writeRedisCache(current, pageSize, loginUser.getId(), 30000, TimeUnit.MILLISECONDS);
        return ResultUtils.success(userVOPage);
    }
